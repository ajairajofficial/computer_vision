...
def init_excel_log(log_path):
    """Initialize or load the Excel log file."""
    if not os.path.exists(log_path):
        wb = Workbook()
        ws = wb.active
        ws.title = "Detections"
        ws.append(["Name", "Timestamp", "Access"])
        wb.save(log_path)
    # No return needed, just ensure file exists

def append_excel_log(log_path, name, access):
    """Append a detection event to the Excel log file."""
...
def main():
    ...
    # Excel log setup
    log_path = os.path.join(script_dir, "detection_log.xlsx")
    init_excel_log(log_path)
            ...
            if all(n == "Unknown" for n in detected_names):
                if not access_denied:
                    ...
                    for n in detected_names:
                        append_excel_log(log_path, n, "Access Denied")
                ...
                if not special_access_granted:
                    ...
                    for n in detected_names:
                        if n == "Ajai_Raj":
                            append_excel_log(log_path, n, "Special Access Granted")
                        else:
                            append_excel_log(log_path, n, "Access Granted")
            else:
